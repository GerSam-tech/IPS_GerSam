...
const Topbar = ({ title, goTo, isCollapsed, toggleSidebar }) => {
    ...
    <div className="topbar" id="topbar">
      ...
      <div className="topbar-actions">
        <button className="tb-btn green" onClick={toggleVoice} style={{ background: isListening ? 'var(--red)' : '', color: isListening ? 'white' : '' }} title="Búsqueda por Voz">
          {isListening ? '🔴 ' : '🎙️ '}<span className="tb-btn-text">{isListening ? 'Escuchando...' : 'Voz'}</span>
        </button>
        <button className="tb-btn wa" onClick={() => window.open('https://web.whatsapp.com/', '_blank')} title="Abrir WhatsApp Web">
          ...
          <span className="tb-btn-text">WhatsApp Web</span>
        </button>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <div className="status-dot"></div>
          <span className="status-dian-text" style={{ fontSize: '.72rem', color: 'var(--green)', fontWeight: 600 }}>DIAN Online</span>
        </div>
        <button className="tb-btn primary" onClick={() => goTo('registro')}>
          <span className="tb-btn-icon">+</span> <span className="tb-btn-text">Nuevo Paciente</span>
        </button>
      </div>
    </div>
  );
